...
function insertFutureOrderDetails(res, cust_id, order_details) {
    ...
    db.query('UPDATE order_detail SET invoice = invoice + 1', (err) => {
        if (err) {
            console.error('Error updating invoice counter: ' + err.message);
            return res.status(500).json({ error: 'Internal server error' });
        }

        // Fetch the updated invoice counter from the database
        db.query('SELECT invoice FROM order_detail', async (err, counterResult) => {
            if (err || !counterResult || counterResult.length === 0) {
                console.error('Error fetching invoice counter');
                return res.status(500).json({ error: 'Internal server error' });
            }

            const invoiceCounter = counterResult[0].value;

            // Generate the invoice number based on the current date and counter
